 ...
 *
 * @MigrateSource(
 *   id="dynamic_lessons_url"
 * )
 */
class DynamicLessonsUrl extends SourcePluginExtension implements ContainerFactoryPluginInterface {
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition, MigrationInterface $migration, MigrateD8ServicesConfigurationInterface $migrateD8ServicesConfiguration) {
    ...
    if (!is_array($configuration['urls'])) {
      $configuration['urls'] = [$this->migrateD8ServicesConfiguration->GetMigrateD8ServiceRestLessonsEndpointUrl()];
    }
